...
import {FlattenedNodesObserver as PolymerFlattenedNodesObserver}
...
class ActiveCampaignForm extends PolymerElement {
	...
	connectedCallback() {
		...
		window._show_error = (...args) => {
			this.instances
				.forEach(instance => instance._showError.call(instance, ...args));
		};
		window._show_thank_you = (...args) => {
			this.instances
				.forEach(instance => instance._showThankYou.call(instance, ...args));
		};
	...
	submit() {
		...
		this._loadScript(`https://${this.accountName}.activehosted.com/proc.php?${urlData}&jsonp=true`);
	}
